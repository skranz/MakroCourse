...
- Die Okun'sche Faustregel (Okun'sches Gesetz):
    ...
    *Eine um 1% höhere Arbeitslosenquote entsprichte einer Reduktion von Y um ca. 2%-3% (relativ zu Y_NAI).*
...
curves:
  okun_curve:
    ...
    colors: lila
    tooltip: Arbeitslosenquote als Funktion des BIP Y gemäß der Okun'schen Faustregel mit c={{c_Okun}} und u_NAI = {{u_NAI}}.
ymarkers:
  u:
    latex: u_t
    to: Y
  ...
  u_NAI:
    latex: u_NAI
    to: Y_NAI
    tooltip: u_NAI ist die "natürliche" Arbeitslosequote, auch NAIRU (Non-Accelerating Inflation Rate of Unemployment) genannt. Sie ist die Arbeitslosenquote, bei der die Inflationsrate \pi_t genau ihren Erwartungen E\pi_t entspricht.
xmarkers:
  Y_NAI:
    tooltip: Y_NAI = {{Y_NAI}} ist das Nachfrageniveau bei dem die Inflationsrate bei ihrem erwarteten Wert verbleibt. NAI steht für 'Non-Accelerating Inflation'.
    latex: Y_NAI
...
-  Beispielsweiser Zusammenhang von BIP Y und Arbeitslosigkeit u gemäß einer Okun'schen Faustregel.
- $u_{NAI}$ = 5% und c = 3.
</td><td>
#< plotpane Okun_pane
width: 320
...
curves:
  pcu:
    ...
    colors: lila
    tooltip: Diese Phillips-Kurve zeigt den Zusammenhang zwischen Inflation und Arbeitslosikeit in einer Periode, für gegebene Inflationserwartungen E\pi_{{.row}} = {{Epi}}.
ymarkers:
  Epi:
    latex: E\pi_{{.row}}
    to: u_NAI
...
<tr><td style="vertical-align: top; width: 35%; padding-top: 0px;">
...
In unserem Beispiel hat die Zentralbank durch den niedrigen Zinssatz von $r_{cb}$={{r_cb*100}}% die Nachfrage auf Y={{Y}} > Y_NAI = {{Y_NAI}} angekurbelt und die Arbeitslosigkeit auf u = {{round(u*100,2)}}% unter das "natürliche" Niveau von u_NAI = {{u_NAI*100}}% gesenkt. Dafür beträgt die Inflationsrate $\pi_t$ = {{round(pi*100,2)}}%, statt der erwarteten $E\pi_t$ = {{Epi*100}}%.
...
sc:
  - Die Arbeitslosenquote bleibt in jeder Periode bei u={{round(u*100,2)}}% und die Inflationsrate bei $\pi$={{round(pi*100,2)}}%
  - Die Arbeitslosenquote bleibt bei u={{round(u*100,2)}}%, aber die Inflationsrate steigt jedes Jahr an.*
  - Die Arbeitslosenquote geht langsam auf Ihr natürliches Niveau $u_NAI$={{u_NAI*100}}% zurück, die Inflationsrate bleibt aber bei 
#>  
...
<td class="leftcol", style="width: 40%;"> {{menu}}{{text}}</td>
