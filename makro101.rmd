...
<td class="leftcol" style="width: 35%;">
...
</td><td class="rightcol" style="width: 35%;">
...
$$
\begin{eqnarray*}
P_t & = & EP_t & \cdot (1+\mu) \cdot ws(Y_t) & \Leftrightarrow \\
...
#. frame Beispiel für Okun'sche Faustregel
...
<td style="vertical-align: top; width: 40%; padding-top: 0px; padding-right: 10px">
...
#. frame Phillipskurve: Arbeitslosigkeit vs Inflation
...
xmarkers:
  u:
    latex: u_t
    to: pi
    tooltip: u_t = {{round(u*100,2)}}% ist die Arbeitslosenquote in dieser Periode.
  u_NAI:
    latex: u_NAI
    to: Epi
    tooltip: |
...
<table class="frametable">
<tr><td class="leftcol" style="width: 35%;">
...
#. frame Inflations-Arbeitslosigkeit-Tradeoff

<table class="frametable">
<tr><td class="leftcol" style="width: 50%;">
...
-> Arbeitslosigkeit auf u = {{round(u*100,2)}}% unter das "natürliche" Niveau von u_NAI = {{u_NAI*100}}% gesenkt. 

-> Aber Inflationsrate $\pi_t$ = {{round(pi*100,2)}}%, statt der erwarteten $E\pi_t$ = {{Epi*100}}%.
...
<table class="frametable">
<tr><td class="leftcol" style="width: 55%;">
...
sc:
  - Die Arbeitslosenquote bleibt in jeder Periode bei {{round(u*100,2)}}% und die Inflationsrate bleibt bei {{round(pi*100,2)}}%.
  - Die Arbeitslosenquote bleibt bei {{round(u*100,2)}}%, aber die Inflationsrate steigt jede Periode immer weiter an.*
  - Die Arbeitslosenquote steigt langsam von {{round(u*100,2)}}% auf ihr natürliches Niveau u_NAI={{u_NAI*100}}% an, die Inflationsrate verbleibt bei {{round(pi*100,2)}}%.
#>  
...
#. success
Genau, die Inflationsrate wird von $\pi_1={{pi_1}}$ auf $\pi_2={{round(pi_2,3)}}$ ansteigen.
#>


#. frame Lohn- Preisspirale t=3
#< precompute
```{r}
...
- Wenn $Y_{t-1} = Y_t = Y_NAI$, steigen die Nominallöhne mit der erwarteten Inflationsrate $E\pi_t$.
- Sei $Y_{t-1} = Y_t > Y_NAI$ 
...
#. frame Einperiodiger Nachfrageboom
#< precompute
```{r}
.whiskers = as.list(exdat[1,])
```
...
sc:
  - Die Arbeitslosenquote springt ab Periode 2 sofort wieder von u_1={{round(u*100,2)}}% auf u_NAI={{round(u_NAI*100,2)}}% aber die Inflation bleibt stets bei pi_1={{round(pi*100,2)}}%.*
  - Die Arbeitslosenquote steigt langsam wieder auf u_NAI={{round(u_NAI*100,2)}}% und die Inflationsrate sinkt langsam wieder auf E\pi_1 = {{round(Epi*100,2)}}% ab.
  - Die Arbeitslosenquote bleibt stets bei u_1={{round(u*100,2)}}% und die Inflationsrate bleibt stets bei pi_1 = {{round(pi*100,2)}}%.
#>  
...
- Auch bei einem Rückgang von $Y_1={{Y}}$ auf Y_NAI bleiben die  Inflationserwartungen auf dem Niveau von $E\pi_t=\pi_{t-1}=...=\pi_1={{pi}}$ bestehen.
- Ein einmaliger kurzer Boom erhöht in unserem Modell dauerhaft die Inflationsrate.
- Um die Inflationsrate wieder zu reduzieren, müsste die Zentralbank die Zinsen erstmal stärker anheben, so dass für eine oder mehrere Perioden wir eine kleine Rezession mit $Y_t < Y_{NAI}$ haben.
...
find: Y_5
#. tell
Angenommen, die Zentralbank möchte in t=2 gleich wieder eine Inflationsrate von $\pi_2=E\pi_1$=2% haben. Welches Nachfrageniveau $Y_2$ muss sie dafür mit ihrem Zinssatz $r_2$ induzieren. Markieren Sie dieses $Y_2$ in dem Abbild links. 
#. success
Genau, wir müssen die Phillipskurve von t=2 betrachten und dass jenige Y implementieren, an dem die resultierende Inflationsrate gleich 2% ist, also $Y_2={{Y_5}}.$
#>
