...
points:
  A:
    xy: [Y_T, pi_T]
    tooltip: |
      Zentralbank hat nur das Ziel, sofort wieder
      die Inflationsrate pi_T = {{perc(pi_T)}}
      herzustellen.
  B:
    xy: [Y_NAI, Epi]
    tooltip: |
      Zentralbank hat nur das Ziel, jede Periode
      das BIP auf dem Niveau Y=Y_NAI zu halten.
#>
...
panes:
  pc_pane:
    ...
    curves:
      cb_pref:
        eq: pi == pi_star + sqrt(-((a_Y * ((Y_NAI - Y)/Y_NAI)^2 - const)/a_pi))
        color: red
