...
Wenn wir  dass das wohlfahrtsoptimale $x^o$ auch diesen Nettogewinn maximiert, d.h. die Firma wird $x=x^o$ wählen.
...
– const ist eine Konstante, die unabhängig von xist
...
B muss A für den entgangenen Gewinn aufgrund der Schadstoffreduktion kompensieren. Da wir nehmen, dass es nach einer etwaigen Ablehnung kein Gegenangebot gibt, hat B hier die vollstädnige Verhandlungsmacht und wird nur $z(x)$ als Zahlung anbieten.
...
b) Lösen Sie Aufgabe a) mit der Variante, dass A ein Recht auf Verschmutzung in Höhe von x_{r} mit x^{o}<x_{r}\leq x^{*} hat. Zeigen Sie, dass weiterhin vertraglich x=x^{o} gewählt wird und sich nur die Zahlungen des B an A verändern.
...
Der resultierende Schadstoffausstoss $x=x^o$ Ergebnis ist unabhängig von der genauen Höhe von $x_r$. Das Verschmutzungsrecht $x_r$ bestimmt aber die Höhe der Zahlung des B an A. Je höher $x_r$, desto mehr muss B den A kompensieren. Die Höhe der Verschmutzungsrechte hate hier keine Effizienzwirkung,  aber immer noch eine Verteilungswirkung zwischen Schädiger A und Geschädigtem B.
#>
...
#< note Lösung
Nun kann A dem B einen Vertrag anbieten, der A erlaubt $x > x_r = 0$ an Schadstoffausstoßen im Gegenzug für eine Kompensationszahlung von $z$ an B. Wenn B den Vertrag ablehnt, wird A das maximale erlaubte $x_r = 0$ ausstoßen und B hat einen Schaden von $S(x_r) = S(0) = 0$.
...
- Betrachten Sie Aufgabe 2.7 a) mit Verhandlungen für den Fall, dass eine Steuer $t \geq 0$ auf die Schadstoffe erhoben wird. Bei welchem Steuersatz wird in der Verhandlungslösung, die wohlfahrtsoptimale Schadstoffmenge x^{o} gewählt?

#< note Lösung

Wie in Aufgabe 2.7 a) kann B so viel verschmutzen wir er möchte, muss jetzt aber eine Steuer $tx$ zahlen. Weiterhin kann B dem A einen Vertrag anbieten, das Verschmutzungsniveau zu reduzieren, im Gegenzug für eine Zahlung z von B and A.
...
Nur im Falle eines Steuersatzes, von $t=0$, d.h. keine Steuern, ist Nutzenmaximierung wieder äquivalent zur Wohlfahrtsmaximierung.
