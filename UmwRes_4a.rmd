...
<h2> Lösungen zu Kapitel 4a:<br></h2>
...
+ Staat setzt Umweltziel $\bar{E}$: Maximal tolerierte Gesamtemissionsmenge

  + $\bar{V}=\bar{E}-\sum_{i=1}^{n}e_{i}^{0}$ angestrebte Gesamtvermeidung

#. frame Statische Ineffizienz von Auflagen

Aufgabe 4.1:

a) Zeigen Sie mit Hilfe eines Lagrange Ansatzes dass für jedes beliebige Umweltziel $\bar{E}$ (und entsprechenden Vermeidungsziel $\bar{V}$ statische Effizienz dann erzielt wird, wenn jede Firma soviel Emission vermeidet, dass die Grenzvermeidungskosten für alle Firmen gleich hoch sind: $$R_{i}'(v_{i}^{o})=\lambda\forall i$$ wobei $\lambda$ der Langrange Multiplikator ist.

#< note Lösung
Wir betrachten das Optimierungsproblem, die Gesamtkosten zu minimieren unter der Nebenbedingung, dass die Gesamtemmissionen $\bar{E}$ nicht überschreiten.

Man könnte das Problem als Funktion der Emissionen $e_i$ oder der Emissionsvermeidungen $v_i$ schreiben. Wir stellen das Problem mit Hilfe der Emmissionsvermeidungen $v_i$ da.
...
$$\sum_{i=1}^{n} v_i \geq \bar{V}$$ 

D.h. die Gesamtvermeidungen müssen mindestens $\bar{V}$ betragen. Da Emmissionsvermeidung mit Kosten verbunden, ist wird diese Nebenbeding im Optimum mit Gleichheit halten.
...
wobei wir die Nebenbedingung so umstellen, dass rechts 0 steht, und wir dann die linke Seite der Nebenbedingung in die Lagrangefunktion einsetzen. Also hier hätten wir

$$L = \sum_{i=1}^{n} R(v_i) - \lambda \left( \sum_{i=1}^{n} v_i - \bar{V} \right)$$

Die Bedingung erster Ordnung für ein Optimum sind dass, die Ableitung der Lagrangefunktion nach den Entscheidungsvariablen $v_i$ gleich 0 sind und alle Nebenbedingungen erfüllt sind.
...
b) Lösen Sie Aufgabe a) für folgendes Beispiel:
...
Ohne Politikeingriff werden die Firmen eine Emmission von $E^0 =  e_1^0 + e_2^0 = 400$ haben. Ein Emmissionsziel von  $\bar{E} = 100$ entspricht also einem Vermeidungsziel von $\bar{V} = E^0 - \bar{E} = 200$.
...
d) Illustrieren Sie grafisch, dass eine Emissionsauflage bei der jede Firma genau eine Schadstoffmenge $\frac{\bar{E}}{n}$ ausstoßen darf, im allgemeinen nicht kosteneffizient ist und stellen Sie den Wohlfahrtsverlust grafisch dar.
...
Wir können einfach nachprüfen, dass tatsächlich die gleichen Emmissionsreduktionen, wie in Aufgabe 4.1 b) herauskommen. Wir haben

$$R'(v_1) = 2 v_1 = 400 \Leftrightarrow v_1 = 200$$
$$R'(v_2) = 4 v_1 = 400 \Leftrightarrow v_2 = 400$$
...
+ Nehmen Sie an der Staat versteigert eine Anzahl $\bar{E}$
...
Diese Bedingung beschreibt implizit, wie viel Reduktionen jede Firma vermeidet als Funktion des Zertifikatepreises z.

Für unser Zahlenbeispiel haben wir für Firma 1:

$$R'(v_1) = 2 v_1 = z$$

Die Reduktion ist somit als Funktion des Zertifikatepreises
...
Da Firma 1 für jede Emmissionseinheit ein Zertifikat braucht, beschreibt dies die Nachfrage nach Emmissionszertifikaten asl Funktion des Zertifikatepreises.
...
Die gesamte Nachfrage nach Emmissionszertifikaten ist dann wie folgt gegeben:

$$E(z)= e_1(z) + e_2(z) = 400 - 0.75 z$$

Im Gleichgewicht auf dem Zertifikatemarkt müssen die Nachfrage $E(z)$ und Angebot $\bar{E}$ von Zertifikaten gleich hoch sein.
$$
\begin{align*}
E(z) &= \bar{E}
400 - 0.75 z &= 100
